...
/*int sdbGetIndexInfo( SdbExecState *sdbState, sdbIndexInfo *indexInfo )
{
   ...
   {
      bson_iterator bsonIter1 = {NULL, 0} ;
      bson_iterator_init(&bsonIter1, &obj) ;

      // for each element in bson object
      while(bson_iterator_next(&bsonIter1))
      {
         const CHAR *bsonKey   = bson_iterator_key(&bsonIter1) ;
         bson_type bsonType = bson_iterator_type(&bsonIter1) ;
         if ( strcmp(bsonKey, "IndexDef") == 0
              && bsonType == BSON_OBJECT )
         {
            bson indexDef ;
            bson_iterator bsonIter2 = {NULL, 0} ;
            bson_init(&indexDef);
            bson_iterator_subobject(&bsonIter1, &indexDef) ;
            bson_iterator_init(&bsonIter2, &indexDef) ;
            while( bson_iterator_next(&bsonIter2) )
            {
               const CHAR *key2   = bson_iterator_key(&bsonIter2) ;
               bson_type type2 = bson_iterator_type(&bsonIter2) ;
               if ( strcmp(key2, "key") == 0 && type2 == BSON_OBJECT )
               {
                  bson keyDef ;
                  bson_iterator bsonIter3 = {NULL, 0} ;
                  int i = 0 ;
                  bson_init(&keyDef);
                  bson_iterator_subobject(&bsonIter2, &keyDef) ;
                  bson_iterator_init(&bsonIter3, &keyDef) ;
                  while ( bson_iterator_next(&bsonIter3) )
                  {
                     const CHAR *key3 = bson_iterator_key(&bsonIter3) ;
   ...
   const CHAR *preference_instance )
{
   ...
   if ( NULL != preference_instance ){
      ...
      {
         bson_append_string( &recordObj, FIELD_NAME_PREFERED_INSTANCE,
                                preference_instance ) ;
      }
      else
      {
         bson_append_int( &recordObj, FIELD_NAME_PREFERED_INSTANCE,
                             intPreferenece_instance ) ;
      }

      rc = bson_finish( &recordObj ) ;
      if ( rc != SDB_OK )
      {
                               ...
                               bson *rescanCondition)
{
   ...
   foreach( cell, planState->qual )
   {
      ...
      if( SDB_OK == rcTmp )
      {
         if ( !bson_is_empty( &subBson ) )
         {
            ...
            bson_append_elements( rescanCondition, &subBson ) ;
         }
      }
   ...
   rc = bson_finish( rescanCondition ) ;
   if ( SDB_OK != rc )
   {
...
void sdbPrintBson( bson *bson, int log_level, const char *label )
{
   ...
   bufferSize = bson_sprint_length( bson ) ;
   p = ( char* )malloc( bufferSize ) ;
   bson_sprint( p, bufferSize, bson ) ;
...
void SdbInitRecordCache()
{
   ...
   for (; i < cache->size; i++ )
   {
      cache->recordArray[i].record = malloc( sizeof( bson ) ) ;
      bson_init( cache->recordArray[i].record ) ;
...
void SdbFiniRecordCache()
{
   ...
   for (; i < cache->size; i++ )
   {
      bson_destroy( cache->recordArray[i].record ) ;
