...
static void sdbDeserializeDocument( Const *constant, bson *document ) ;
...
static INT32 sdbAppendConstantValue( bson *bsonObj, const char *keyName,
                                     Const *constant, INT32 isUseDecimal ) ;


static UINT64 sdbCreateBsonRecordAddr(  ) ;
static bson* sdbGetRecordPointer( UINT64 record_addr ) ;

static void sdbGetColumnKeyInfo( SdbExecState *fdw_state ) ;

static bool sdbIsShardingKeyChanged( SdbExecState *fdw_state, bson *oldBson,
      bson *newBson ) ;

static UINT64 bson_iterator_getusecs( bson_iterator *ite ) ;

static INT32 sdbOperExprParamVar( OpExpr *expr, SdbExprTreeState *expr_state,
                           bson *condition, ExprContext *exprContext ) ;
...
static INT32 sdbBooleanTestIsNotExpr( const char *columnName, bson *condition,
                                      BOOLEAN value ) ;
...
static INT32 sdbGenerateFilterCondition( Oid foreign_id, RelOptInfo *baserel,
                                         INT32 isUseDecimal, bson *condition ) ;
...
void initSdbExecState( SdbExecState *sdbExecState )
{
   ...
   bson_init( &sdbExecState->queryDocument ) ;
   bson_finish( &sdbExecState->queryDocument ) ;
}
...
SdbExecState *deserializeSdbExecState( List *sdbExecStateList )
{
   ...
   bson_destroy( &fdwState->queryDocument ) ;
...
int sdbSetBsonValue( bson *bsonObj, const char *name, Datum valueDatum,
         Oid columnType, INT32 columnTypeMod, INT32 isUseDecimal )
{
   ...
   switch( columnType )
   {
      case INT2OID :
      {
         ...
         bson_append_int( bsonObj, name, ( INT32 )value ) ;
         break ;
      }
      ...
      case INT4OID :
      {
         ...
         bson_append_int( bsonObj, name, value ) ;
         break ;
      }
      ...
      case INT8OID :
      {
         ...
         bson_append_long( bsonObj, name, value ) ;
         break ;
      }
      ...
      case FLOAT4OID :
      {
         ...
         bson_append_double( bsonObj, name, ( FLOAT64 )value ) ;
         break ;
      }
      ...
      case FLOAT8OID :
      {
         ...
         bson_append_double( bsonObj, name, value ) ;
         break ;
      }
      ...
      case NUMERICOID :
      {
         ...
         if ( isUseDecimal )
         {
            ...
            {
               bson_append_decimal3( bsonObj, name, value ) ;
            }
            else
            {
               ...
               bson_append_decimal2( bsonObj, name, value, precision, scale ) ;
            }
         }
         else
         {
            bson_append_string( bsonObj, name, value ) ;
         }
      ...
      case BOOLOID :
      {
         ...
         bson_append_bool( bsonObj, name, value ) ;
         break ;
      }
      ...
      case TEXTOID :
      {
         ...
         if ( strcmp( name, SDB_KEYNAME_REGEX ) == 0 )
         {
            ...
            if ( NULL != regexFormat )
            {
               bson_append_string( bsonObj, name, regexFormat ) ;
               pfree( regexFormat ) ;
            }
            else
            {
         ...
         {
            bson_append_string( bsonObj, name, outputString ) ;
         }
      ...
      case NAMEOID :
      {
         ...
         bson_append_oid( bsonObj, name, &bsonObjectId ) ;
         pfree( outputString ) ;
         break ;
      }
      ...
      case DATEOID :
      {
         ...
         bson_append_date( bsonObj, name, valueMilliSecs ) ;
         break ;
      }
      ...
      case BYTEAOID :
      {
         ...
         bson_append_binary( bsonObj, name, BSON_BIN_BINARY, buff, len ) ;
         break ;
      }
      ...
      case 1001:
      {
         ...
         bson_append_start_array( bsonObj, name ) ;
...
bson *sdbGetRecordPointer( UINT64 record_addr )
{
   SdbRecordCache *cache = SdbGetRecordCache() ;
                             ...
                             SdbCLStatistics *clStat )
{
   ...
   bson_append_string( &condition, "Name", fullCollectionName->data ) ;
   rc = bson_finish( &condition ) ;
   if ( SDB_OK != rc )
   {
      ereport( ERROR, ( errcode( ERRCODE_FDW_ERROR ),
               errmsg( "bson_finish failed:rc=%d", rc ) ) ) ;
      goto error ;
   }

   bson_append_string( &selector, SDB_SHARDINGKEY_NAME, "" ) ;
   rc = bson_finish( &selector ) ;
   if ( SDB_OK != rc )
   {
      ereport( ERROR, ( errcode( ERRCODE_FDW_ERROR ),
            errmsg( "bson_finish failed:rc=%d", rc ) ) ) ;
   ...
   type = bson_find( &ite, &ShardingKey, SDB_SHARDINGKEY_NAME ) ;
   if ( BSON_OBJECT == type )
   {
      bson tmpValue ;
      bson_init( &tmpValue ) ;
      bson_iterator_subobject( &ite, &tmpValue ) ;
      bson_iterator_init( &ite, &tmpValue ) ;
      while ( bson_iterator_next( &ite ) )
      {
         const CHAR *bsonKey = bson_iterator_key( &ite ) ;

         strncpy( clStat->shardingKeys[clStat->keyNum],
               bsonKey, SDB_MAX_KEY_COLUMN_LENGTH - 1 ) ;
         clStat->keyNum++ ;
      }
      bson_destroy( &tmpValue ) ;
   }
...
bool sdbIsShardingKeyChanged( SdbExecState *fdw_state, bson *oldBson,
      bson *newBson )
{
   int i = 1 ;
   bson_iterator oldIte ;
   bson oldSubBson ;
   bson_type oldType ;
   int oldSize ;

   bson_iterator newIte ;
   bson newSubBson ;
   bson_type newType ;
   int newSize ;
   ...
   {
      bson_init( &newSubBson ) ;
      newType = bson_find( &newIte, newBson, fdw_state->key_name[i] ) ;
      if ( BSON_EOO == newType )
      {
         /* not in the newBson, so no change */
         continue ;
      }
      bson_append_element( &newSubBson, NULL, &newIte ) ;
      bson_finish( &newSubBson ) ;

      bson_init( &oldSubBson ) ;
      oldType = bson_find( &oldIte, oldBson, fdw_state->key_name[i] ) ;
      if ( BSON_EOO == oldType )
      {
         ...
         bson_destroy( &newSubBson ) ;
         bson_destroy( &oldSubBson ) ;
         return true ;
      }
      bson_append_element( &oldSubBson, NULL, &oldIte ) ;
      bson_finish( &oldSubBson ) ;

      oldSize = bson_size( &oldSubBson ) ;
      newSize = bson_size( &newSubBson ) ;
      if ( ( oldSize != newSize )||
...
UINT64 bson_iterator_getusecs( bson_iterator *ite )
{
   bson_type type = bson_iterator_type( ite ) ;
   if ( BSON_DATE == type )
   {
      return ( bson_iterator_date( ite )* SDB_MSECS_TO_USECS ) ;
   }
   else
   {
      bson_timestamp_t timestamp = bson_iterator_timestamp( ite ) ;
                           ...
                           bson *condition, ExprContext *exprContext )
{
   ...
   bson_append_start_object( condition, columnName ) ;
                         ...
                         bson *condition )
{
   ...
   bson_init( &temp1 ) ;
   bson_append_string( &temp1, "$field", columnName2 ) ;
   bson_finish( &temp1 ) ;

   bson_init( &temp2 ) ;
   bson_append_bson( &temp2, sdbOpName, &temp1 ) ;
   bson_finish( &temp2 ) ;
   bson_destroy( &temp1 ) ;

   bson_append_bson( condition, columnName1, &temp2 ) ;
   bson_destroy( &temp2 ) ;
                   ...
                   bson *condition, ExprContext *exprContext )
{
   ...
   else if ( SDB_VAR_CONST == argType )
   {
      ...
      if ( need_not_condition )
      {
         ...
         bson_copy( &temp, condition ) ;

         bson_destroy( condition ) ;
         bson_init( condition ) ;

         bson_append_start_array( condition, "$not" ) ;
         bson_append_bson( condition, "0", &temp ) ;
                            ...
                            bson *condition )
{
   ...
   foreach( cell, scalaExpr->args )
   {
      ...
      if ( T_Const == oprarg->type )
      {
         ...
         bson_finish( &temp ) ;
         bson_append_bson( condition, columnName, &temp ) ;
         bson_destroy( &temp ) ;
                  ...
                  bson *condition )
{
   ...
   columnName = get_relid_attribute_name( expr_state->foreign_table_id,
                                          columnId ) ;
   bson_append_bool( condition, columnName, TRUE ) ;
...
INT32 sdbBooleanTestIsNotExpr( const char *columnName, bson *condition,
                               BOOLEAN value )
{
   INT32 rc = SDB_OK ;
   bson tmpCondition ;
   bson subCondition ;
   bson_init( &subCondition ) ;
   bson_init( &tmpCondition ) ;

   rc = bson_append_bool( &subCondition, columnName, value ) ;
   if ( SDB_OK != rc )
   {
      elog( WARNING, "bson_append_bool failed:rc=%d", rc ) ;
      goto error ;
   }

   rc = bson_finish( &subCondition ) ;
   if ( SDB_OK != rc )
   {
      elog( WARNING, "bson_finish subCondition failed:rc=%d", rc ) ;
      goto error ;
   }

   rc = bson_append_start_array( &tmpCondition, "$not" ) ;
   if ( SDB_OK != rc )
   {
      elog( WARNING, "bson_append_start_array failed:rc=%d", rc ) ;
      goto error ;
   }

   rc = bson_append_bson( &tmpCondition, "0", &subCondition ) ;
   if ( SDB_OK != rc )
   {
      elog( WARNING, "bson_append_bson failed:rc=%d", rc ) ;
      goto error ;
   }

   rc = bson_append_finish_array( &tmpCondition ) ;
   if ( SDB_OK != rc )
   {
      elog( WARNING, "bson_append_finish_array failed:rc=%d", rc ) ;
      goto error ;
   }

   rc = bson_finish( &tmpCondition ) ;
   if ( SDB_OK != rc )
   {
      elog( WARNING, "bson_finish failed:rc=%d", rc ) ;
      goto error ;
   }

   rc = bson_append_elements( condition, &tmpCondition ) ;
   if ( SDB_OK != rc )
   {
      elog( WARNING, "bson_append_elements failed:rc=%d", rc ) ;
      goto error ;
   }

done:
   bson_destroy( &subCondition ) ;
   bson_destroy( &tmpCondition ) ;
                          ...
                          bson *condition )
{
   ...
   {
      bson_append_bool( condition, columnName, FALSE ) ;
   }
                       ...
                       bson *condition )
{
   ...
   columnName    = get_relid_attribute_name( expr_state->foreign_table_id,
                                                 columnId ) ;
   bson_init( &isNullcondition ) ;
   switch ( ntest->nulltesttype )
   {
      case IS_NULL:
         bson_append_int( &isNullcondition, "$isnull", 1 ) ;
         break ;
      case IS_NOT_NULL:
         bson_append_int( &isNullcondition, "$isnull", 0 ) ;

         break ;
      default:
         rc = SDB_INVALIDARG ;
         bson_destroy( &isNullcondition ) ;
         elog( DEBUG1, "nulltesttype error:type=%d", ntest->nulltesttype ) ;
         goto error ;
   }

   bson_finish( &isNullcondition ) ;
   bson_append_bson( condition, columnName, &isNullcondition ) ;
   bson_destroy( &isNullcondition ) ;
                        ...
                        bson *condition, ExprContext *exprContext )
{
   ...
   bson_init( &tmpCondition ) ;
   bson_append_start_array( &tmpCondition, key ) ;
   foreach( cell, boolexpr->args )
   {
      INT32 rcTmp = SDB_OK ;
      Node *bool_arg = ( Node * )lfirst ( cell ) ;
      bson sub_condition ;
      bson_init( &sub_condition ) ;
      rcTmp = sdbRecurExprTree( bool_arg, &tmp_expr_state, &sub_condition,
                                exprContext ) ;
      bson_finish( &sub_condition ) ;

      if ( SDB_OK == rcTmp )
      {
         if ( !bson_is_empty( &sub_condition ) )
         {
            CHAR arrayIndex[SDB_MAX_KEY_COLUMN_LENGTH + 1] = "" ;
            sprintf( arrayIndex, "%d", count ) ;
            bson_append_bson( &tmpCondition, arrayIndex,
                                    &sub_condition ) ;
            count++ ;
         }
      }
      ...
      bson_destroy( &sub_condition ) ;
   }

   bson_append_finish_array( &tmpCondition ) ;
   bson_finish( &tmpCondition ) ;
   if ( 0 == tmp_expr_state.total_unsupport_count ||
        0 == strcmp( key, "$and" ) )
   {
      if ( count > 0 )
      {
         bson_append_elements( condition, &tmpCondition ) ;
      }
   }
                                   ...
                                   INT32 isUseDecimal, bson *condition )
{
   ...
   foreach( cell, baserel->baserestrictinfo )
   {
      ...
      if( SDB_OK == rcTmp )
      {
         if ( !bson_is_empty( &subBson ) )
         {
            bson_append_elements( condition, &subBson ) ;
         }
      }

      bson_destroy( &subBson ) ;
 ...
 */
Const *sdbSerializeDocument( bson *document )
{
 ...
 */
void sdbDeserializeDocument( Const *constant, bson *document )
{
   if ( constant->constisnull )
   {
      bson_init( document ) ;
...
INT32 sdbAppendConstantValue ( bson *bsonObj, const char *keyName,
                               Const *constant, INT32 isUseDecimal )
{
   int rc = SDB_OK ;
   ...
   if ( constant->constisnull )
   {
      ...
      bson_append_int( bsonObj, "$isnull", 1 ) ;
      return rc ;
   }
...
static BOOLEAN sdbColumnTypesCompatible( bson_type bsonType,
                                         Oid columnTypeId, INT32 isUseDecimal )
{
   ...
   case NUMERICOID :
   {
      if ( !isUseDecimal )
      {
         if( BSON_STRING == bsonType || BSON_DOUBLE == bsonType ||
             BSON_INT == bsonType || BSON_LONG == bsonType )
         {
            compatibleType = TRUE ;
         }
      }
   ...
   case TIMESTAMPTZOID :
   {
      if ( ( BSON_TIMESTAMP == bsonType )|| ( BSON_DATE == bsonType ) )
         compatibleType = TRUE ;
      break ;
   }
   case BYTEAOID :
   {
      if ( ( BSON_BINDATA == bsonType ) )
      {
         compatibleType = TRUE ;
      }
      break ;
                             ...
                             INT32 columnTypeMod )
{
   ...
   case NUMERICOID :
   {
      ...
      else if ( bsonType == BSON_DECIMAL )
      {
         ...
         rc = bson_iterator_decimal( bsonIterator, &decimal ) ;
         if ( SDB_OK != rc )
         {
   ...
   case DATEOID :
   {
      INT64 utcUsecs       = bson_iterator_getusecs( bsonIterator ) ;
   ...
   case BYTEAOID :
   {
      const CHAR *buff = bson_iterator_bin_data( bsonIterator ) ;
      INT32 len        = bson_iterator_bin_len( bsonIterator ) ;
   ...
   case JSONOID :
   {
      ...
      length = bson_sprint_length_iterator( bsonIterator ) ;
      ...
      pTemp = pBuff ;
      bson_sprint_iterator( &pTemp, &length, bsonIterator, '"' ) ;
                                   ...
                                   Oid valueTypeId, INT32 isUseDecimal )
{
   ...
   while( bson_iterator_next( &bsonSubIterator ) )
   {
      ...
      if ( BSON_ARRAY == bsonType && OidIsValid( valueTypeId ) )
      {
...
/*
      if ( BSON_ARRAY == bsonType )
      {
         columnValueArray[arrayIndex] = sdbColumnValueArray( &bsonSubIterator,
                                                              valueTypeId ) ;
      }
      else
      {
                              ...
                              bool *columnNulls )
{
   ...
   while( bson_iterator_next( &bsonIterator ) )
   {
      ...
      if( BSON_OBJECT == bsonType && JSONOID != columnTypeId )
      {
         bson subObject ;
         bson_init( &subObject ) ;
         bson_iterator_subobject( &bsonIterator, &subObject ) ;
         sdbFillTupleSlot( &subObject, bsonFullKey, columnMappingHash,
                                       ...
                                       List *restrictionClauses )
{
   ...
   bson_destroy( &fdw_state->queryDocument ) ;
...
static TupleTableSlot * SdbIterateForeignScan( ForeignScanState *scanState )
{
   ...
   if ( SDB_INVALID_HANDLE == executionState->hCursor )
   {
      rc = sdbQuery1( executionState->hCollection, &executionState->queryDocument, NULL,
                      NULL, NULL, 0, -1, QUERY_WITH_RETURNDATA,
                                    ...
                                    FLOAT64 *totalDeadRowCount )
{
   ...
   if ( rc )
   {
      ...
      bson_dispose( &fdw_state->queryDocument ) ;
      goto error ;
   }
      ...
      TupleTableSlot *slot, TupleTableSlot *planSlot )
{
   ...
   bson_init( &bsonCondition ) ;
   original = sdbGetRecordPointer( fdw_state->bson_record_addr ) ;
   for ( i = 0 ; i < fdw_state->key_num ; i++ )
   {
      bson_iterator ite ;
      bson_find( &ite, original, fdw_state->key_name[i] ) ;
      bson_append_element( &bsonCondition, NULL, &ite ) ;
   }
   bson_finish( &bsonCondition ) ;

   //delete the bson from the sdb
   rc = sdbDelete( fdw_state->hCollection, &bsonCondition, NULL ) ;
   if ( rc != SDB_OK )
   {
      ...
      TupleTableSlot *slot, TupleTableSlot *planSlot )
{
      ...
      if ( !isnull )
      {
         ...
         if ( SDB_OK != rc )
         {
            ereport ( WARNING, ( errcode( ERRCODE_FDW_INVALID_DATA_TYPE ),
                      errmsg( "convert value failed:key=%s",
                      fdw_state->pgTableDesc->cols[i].pgname ) ) ) ;
            bson_destroy( &bsonTempValue ) ;
            return NULL ;
         }
      }
   }
   bson_finish( &bsonTempValue ) ;
   ...
   if ( sdbIsShardingKeyChanged( fdw_state, original, &bsonTempValue ) )
   {
      bson_destroy( &bsonCondition ) ;
      bson_destroy( &bsonTempValue ) ;
